...
func TestFilterResource(t *testing.T) {
	...
	var tests = []struct {
		...
		{
			name: "filter role or serviceaccount failed wiht different namespace",
			input: &policyv1alpha1.ServiceAccountAccess{
				ObjectMeta: metav1.ObjectMeta{
					Name:      "sa1",
					Namespace: "my-namespace",
...
func TestSyncRules(t *testing.T) {
	...
	var tests = []struct {
		...
		{
			name:  "reconcile failed casue acc not found",
		...
		{
			name:  "reconcile failed casue deletionTimestamp not nil",
