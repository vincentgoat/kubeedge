...
func TestTokenGenerateAndValidate(t *testing.T) {
	...
	for k, tc := range testCases {
		...
		m := &client.CacheManager{Indexers: make(map[reflect.Type]cache.Indexer)}
		getter := NewGetterFromClient(corelisters.NewSecretLister(
			client.GetOrCreateIndexer(m, &v1.Secret{})),
		...
		for _, obj := range tc.InitObjs {
			err := client.GetOrCreateIndexer(m, obj).Add(obj)
			if err != nil {
				t.Fatalf("TestCase %v add indexer failed: %v", k, err)
			}
		}
		authn := JWTTokenAuthenticator(client.GetOrCreateIndexer(m, &authenticationv1.TokenRequest{}),
			[]string{serviceaccount.LegacyIssuer, "bar"}, tc.Keys, auds, serviceaccount.NewLegacyValidator(tc.Client != nil, getter))
